...
const ApprovedModerationPage = () => {
    ...
    const debouncedSearch = useCallback(
        debounce(async (searchTerm) => {
            const response = await dispatch(
                getAssetArtCardsThunk({ status: 'approved', search: searchTerm, page: 1, limit: 10 })
            );
            if (response.data) {
                setPaginatedData({
                    data: response.data,
    ...
    const handleRefresh = async () => {
        ...
        const response = await dispatch(getAssetArtCardsThunk({ status: 'approved', page: 1, limit: 10 }));
        if (response.data) {
            setPaginatedData({
                data: response.data,
                currentPage: response.page,
    ...
    const handleNextPage = async () => {
        const response = await dispatch(
            getAssetArtCardsThunk({ status: 'approved', page: paginatedData.currentPage + 1, limit: 10 })
        );
    ...
    return (
        <PageContainer title="Art Cards" description="this is requests">
            <Breadcrumb title="Art Cards Application" subtitle="List approved requests" />
            <AppCard>
                <Box
                    sx={{
