...
const PendingModerationPage = () => {
    ...
    const debouncedSearch = useCallback(
        debounce(async (searchTerm) => {
            const response = await dispatch(
                getAssetArtCardsThunk({ status: 'pending', search: searchTerm, page: 1, limit: 10 })
            );
            if (response.data) {
                setPaginatedData({
                    data: response.data,
    ...
    const handleRefresh = async () => {
        ...
        const response = await dispatch(getAssetArtCardsThunk({ status: 'pending', page: 1, limit: 10 }));
        if (response.data) {
            setPaginatedData({
                data: response.data,
                currentPage: response.page,
    ...
    const handleNextPage = async () => {
        const response = await dispatch(
            getAssetArtCardsThunk({ status: 'pending', page: paginatedData.currentPage + 1, limit: 10 })
        );
